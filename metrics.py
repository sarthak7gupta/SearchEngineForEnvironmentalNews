...
class Metrics:
    def __init__(self,elasticSearchHost,elasticSearchPort,elasticSearchIndex,columns):
        self.url = "http://"+elasticSearchHost+":"+str(elasticSearchPort)+"/"+elasticSearchIndex+"/_search?pretty"
        self.cols = columns
    ...
    def getElasticSearchResults(self,query):
        ...
        formattedElastic = self.formatResultElastic(elastic_results.json()['hits']['hits'],self.cols)    
    ...
    #needs to be modified after integration
    ...
    def precision(self,query,docs):
        ...
        numRelevantItemsRetreived = len(self.commonDocs(formattedElastic,formattedDocs))
    ...
    def MAP(self,queriesSet,docsSet,k):
        elasticResultsSet = [self.getElasticSearchResults(query) for query in queriesSet]
...
mymetrics = Metrics("54.157.12.8",9200,"env_news",["URL","MatchDateTime","Station","Show","IAShowID","IAPreviewThumb","Snippet"])
